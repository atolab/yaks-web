...
A backend using Yaks main memory.
It's not persistent: as soon as the Yaks service stops, all the keys/values stored in this backend are lost.

- **plugin file**: *none - the memory backend is built-in with Yaks.*
...
- **plugin file**: yaks-be-sqlite3.cmxs
...
- **storage creation properties**:
  ...
  - **table** : *optional* - the name of the SQL table to use for storage.
    If not specified, Yaks will create a new table.
  ...
  - **key_size** : *optional* - the size of the `k` VARCHAR that Yaks will use for table creation. Default value: 3072.
...
```bash
curl -X PUT -d '{"lib":"sqlite3","url":"sqlite3:///tmp/sqlite3-yaks.db"}' http://localhost:8000/@/local/plugins/yaks/backend/sqlite3
```
Example of a SQLite3 storage creation using the REST API:
```bash
curl -X PUT -d '{"selector":"/demo/example/sql/**","table":"mytable","key_size":"100","on_dispose":"truncate"}' http://localhost:8000/@/local/plugins/yaks/backend/sqlite3/storage/mytable
```
...
- **plugin file**: yaks-be-postgresql.cmxs
- **initialization properties**:
  - **lib**=postgresql
...
```bash
curl -X PUT -d '{"lib":"postgresql","url":"postgresql://localhost/mydb"}' http://localhost:8000/@/local/plugins/yaks/backend/psql
```
Example of a PostgreSQL storage creation using the REST API:
```bash
curl -X PUT -d '{"selector":"/demo/example/sql/**","table":"mytable","key_size":"100","on_dispose":"truncate"}' http://localhost:8000/@/local/plugins/yaks/backend/psql/storage/mytable
```
...
- **plugin file**: yaks-be-mariadb.cmxs
- **initialization properties**:
  - **lib**=mariadb
...
```bash
curl -X PUT -d '{"lib":"mariadb","url":"mariadb://localhost/mydb"}' http://localhost:8000/@/local/plugins/yaks/backend/mariadb
```
Example of a MariaDB storage creation using the REST API:
```bash
curl -X PUT -d '{"selector":"/demo/example/sql/**","table":"mytable","key_size":"100","on_dispose":"truncate"}' http://localhost:8000/@/local/plugins/yaks/backend/mariadb/storage/mytable
```
...
Each Yaks path to store will map to a an InfluxDB 
...
[point](https://docs.influxdata.com/influxdb/v1.7/concepts/key_concepts/#point) reusing the timestamp set by Yaks
(but with a precision of nanoseconds). The encoding and the original Yaks timestamp are used as tags within the point.

- **plugin file**: yaks-be-influxdb.cmxs
- **initialization properties**:
  - **lib**=influxdb
...
- **storage creation properties**:
  ...
  - **db** : *optional* - the name of the database to use for this storage.
    If not specified, Yaks will create a new database.
...
```bash
curl -X PUT -d '{"lib":"influxdb","url":"http://localhost:8086"}' http://localhost:8000/@/local/plugins/yaks/backend/influxdb
```
Example of a InfluxDB storage creation using the REST API:
```bash
curl -X PUT -d '{"selector":"/demo/example/influxdb/**","db":"mydb","on_dispose":"DropAllSeries"}' http://localhost:8000/@/local/plugins/yaks/backend/influxdb/storage/mydb
```
